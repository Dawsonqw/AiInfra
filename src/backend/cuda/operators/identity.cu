...
__global__ void identity_kernel(const float* input, float* output, std::size_t count) {
...
void IdentityOperator::run(const std::vector<Tensor*>& inputs,
                           const std::vector<Tensor*>& outputs,
                           cudaStream_t stream) const {
    if (inputs.size() != 1 || outputs.size() != 1 || inputs[0]->desc().dtype != DataType::Float32 ||
        outputs[0]->desc().dtype != DataType::Float32) {
        throw std::invalid_argument("CUDA Identity currently supports one float32 tensor");
    }
    const auto count = inputs[0]->desc().numel();
    identity_kernel<<<static_cast<unsigned>((count + 255) / 256), 256, 0, stream>>>(
